...
class TestMarkdownDecomposition:
    ...
    def test_generate_problem_name(self):
        ...
        assert generate_problem_name("Create REST API (with auth)") == "create_rest_api_with_auth"
    ...
    def test_decompose_to_markdown(self, mock_find_files, mock_run_prompt):
        ...
        # Create actual files
        ui_dir = self.work_dir / "build_ui"
        backend_dir = self.work_dir / "setup_backend"
        integrate_dir = self.work_dir / "integrate_components"
        ...
        assert "markdown files" in call_args.lower()
...
class TestMarkdownIntegration:
    ...
    @patch.object(ClaudeClient, 'run_prompt')
    @patch('src.markdown_utils.find_subproblem_files')
    def test_decompose_only_phase(self, mock_find_files, mock_run_prompt):
...
"""
        ...
        # Create actual files
        data_dir = self.work_dir / "data_model"
        api_dir = self.work_dir / "api_layer"
        ...
        data_file = data_dir / "problem.md"
        api_file = api_dir / "problem.md"
        frontend_file = frontend_dir / "problem.md"
